...
from dmpo_trainer_efficient_tokenwise import DPOMultiTrainer #fastchat.train.
...
def mask_labels(conversation, target, tokenizer, conv, gamma=0.7):
    ...
    # coefficient = torch.ones_like(target)
    index = []
    ...
    for i, turn in enumerate(turns):
        ...
        # if i!=0:
        #     t = i - 1
        #     gpt_len = len(tokenizer(parts[1]).input_ids) - 1
        #     alpha = gamma**t * (1-gamma**(num_rounds-1-t)) /  (1-gamma**(num_rounds-1))
        #     coefficient[cur_len + instruction_len : cur_len + turn_len] = alpha
        if i!=0:
            index.append(cur_len + instruction_len)
