...
from fastchat.conversation import SeparatorStyle #.fastchat_eto
...
def preprocess(
    ...
    #print(f'conversations: {conversations[0]}')

    # Tokenize conversations
    input_ids = tokenizer( # tokenizer将文本转化为模型输入的数据格式
        conversations,
        return_tensors="pt",
        padding="max_length",
        max_length=tokenizer.model_max_length,
        truncation=True,
    ).input_ids # 编码结果存储在 input_ids 变量中，是一个 PyTorch 张量
    ...
    #conv.sep2 = "<sep>"  #"<|eot_id|><|begin_of_text|>"
    # Mask targets. Only compute loss on the assistant outputs.
    conv.sep2 = '<|start_header_id|>user<|end_header_id|>\n\n'
    for conversation, target in zip(conversations, targets):
        total_len = int(target.ne(tokenizer.pad_token_id).sum())

        turns = conversation.split(conv.sep2) # conv.sep2
        num_rounds = len(turns) - 1
        cur_len = 1
        target[:cur_len] = IGNORE_TOKEN_ID
        for i, turn in enumerate(turns): # 从0开始
            if turn == "":
                break
            #turn += sep
...
def train():
    ...
    print(f'tokenizer_after: {tokenizer}')
