...
class DPOMultiTrainer(Trainer):
    ...
    def compute_reference_log_probs(self, padded_batch: Dict) -> Dict:
        # ^#print("compute_reference_log_probs")
    ...
    def concatenated_inputs(
        ...
        for k in batch:
            if k.startswith("chosen") and isinstance(batch[k], torch.Tensor):
                ...
                if "labels" in k or is_encoder_decoder:
                    ...
                    pad_value = 0
                elif k.endswith("_coefficient"):
                    pad_value = 0
                concatenated_key = k.replace("chosen", "concatenated")
                concatenated_batch[concatenated_key] = pad_to_length(batch[k], max_length, pad_value=pad_value)
                #print(f'shape of k: {concatenated_batch[concatenated_key].shape}')
        for k in batch:
        ...
        ###print("****************************************^^^^^^^^^^^^^^^^^^^^^^concarenated_input&&&&&concatenated_batch")
    ...
    def dpo_loss(
        ...
        Args:
            ...
            reference_free: If True, we ibgnore the _provided_ reference model and implicitly use a reference model that assigns equal probability to all responses.
    ...
    def get_batch_logps(
        ...
        coefficient: torch.FloatTensor,
        average_log_prob: bool = False,
        label_pad_token_id: int = -100,
        is_encoder_decoder: bool = False,
    ) -> torch.FloatTensor:
        # ^#print("get_batch_logps")
        ...
        if not is_encoder_decoder:
            ...
            coefficient = coefficient[:, 1:].clone()
            logits = logits[:, :-1, :]
        ...
        if average_log_prob:
            return (per_token_logps * loss_mask * coefficient).sum(-1) / loss_mask.sum(-1)
        else:
            return (per_token_logps * loss_mask * coefficient).sum(-1)

    def concatenated_forward(
        self, model: nn.Module, batch: Dict[str, Union[List, torch.LongTensor]]
    ) -> Tuple[torch.FloatTensor, torch.FloatTensor, torch.FloatTensor, torch.FloatTensor]:
        # ^#print("concatenated_forward")
        """Run the given model on the given batch of inputs, concatenating the chosen and rejected inputs together.

        We do this to avoid doing two forward passes, because it's faster for FSDP.
        """
        ###print("********************************^^^^^^^^^^^^^^^^^^^^^^^^^^^^^concatenated_forward&&&&&batch***************************")
        ##print(batch)
        ...
        all_logps = self.get_batch_logps(
            ...
            concatenated_batch["concatenated_coefficient"],
    ...
    def get_batch_loss_metrics(
        ...
        batch['chosen_coefficient'] = torch.ones_like(batch['chosen_labels']).type(torch.FloatTensor)
        for i, index_list in enumerate(batch['chosen_index']):
            num_round = len(index_list)//2
            num_token = sum([index_list[2*k+1] - index_list[k] for k in range(num_round)])
            cumu_j = 0
            for j in range(num_round):
                start = index_list[2*j]
                end = index_list[2*j+1]
                temp = torch.arange(cumu_j, cumu_j + end - start).to(batch['chosen_labels'].device)
                if self.gamma<1:
                    alpha = self.gamma**temp * (1-self.gamma**(num_token-temp)) /  (1-self.gamma**(num_token))
                elif self.gamma==1:
                    alpha = (num_token-temp)/num_token
                batch['chosen_coefficient'][i][start:end] = alpha
                cumu_j += end - start
                
        batch['rejected_coefficient'] = torch.ones_like(batch['rejected_labels']).type(torch.FloatTensor)
        for i, index_list in enumerate(batch['rejected_index']):
            num_round = len(index_list)//2
            num_token = sum([index_list[2*k+1] - index_list[k] for k in range(num_round)])
            cumu_j = 0
            for j in range(num_round):
                ...
                temp = torch.arange(cumu_j, cumu_j + end - start).to(batch['rejected_labels'].device)
                if self.gamma<1:
                    alpha = self.gamma**temp * (1-self.gamma**(num_token-temp)) /  (1-self.gamma**(num_token))
                elif self.gamma==1:
                    alpha = (num_token-temp)/num_token
                    
                print(f"target:{batch['rejected_coefficient'][i][start:end].shape}, i target:{batch['rejected_coefficient'][i].shape}; start:{start}, end:{end}  {batch['rejected_coefficient'][i][start:end]}")
                print(alpha.shape)
                
                batch['rejected_coefficient'][i][start:end] = alpha
                cumu_j += end - start
