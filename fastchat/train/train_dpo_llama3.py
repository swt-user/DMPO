...
from dpo_trainer import DPOMultiTrainer #fastchat.train.
...
def mask_labels(conversation, target, tokenizer, conv):
    if conv.sep_style == SeparatorStyle.ADD_COLON_TWO:
        ...
        sep = conv.sep + conv.roles[1] + " "   # sep = "gpt "
    ...
    conv.sep2 = '<|start_header_id|>user<|end_header_id|>\n\n'
    turns = conversation.split(conv.sep2) # llama 中 sep2 为 "</s><s>" 表示这个段落的结束
    num_rounds = len(turns) - 1 
    # turn表示一个例子中的一轮对话
    cur_len = 1
    ...
    for i, turn in enumerate(turns):
        ...
        turn_len = len(tokenizer(turn).input_ids) - 1 # 将<|eot_id|>设置为pad_token后，需要减去turn中<|eot_id|>的个数
        if turn == '<|begin_of_text|>':
            instruction_len = len(tokenizer('<|begin_of_text|><|start_header_id|>user<|end_header_id|>\n\n').input_ids) - 1
            target[cur_len : cur_len + instruction_len] = IGNORE_TOKEN_ID
            cur_len += instruction_len
            continue
        
        parts = turn.split(sep) # llama中sep为“ “
        ...
        if conv.sep2 == '</s>':
            ...
            cur_len += turn_len + 3
        elif conv.sep2 == '<|start_header_id|>user<|end_header_id|>\n\n':
            cur_len += turn_len + len(tokenizer('<|start_header_id|>user<|end_header_id|>\n\n').input_ids) - 1
        else:
            raise NotImplementedError
    ...
    cur_len -= (len(tokenizer('<|start_header_id|>user<|end_header_id|>\n\n').input_ids) - 1)
    cur_len -= num_rounds * 2
    target[cur_len:] = IGNORE_TOKEN_ID
    ...
    if cur_len < tokenizer.model_max_length:
        if cur_len != total_len:
            # z = target.clone()
            # z = torch.where(z == IGNORE_TOKEN_ID, tokenizer.unk_token_id, z) # 将 target 中的 IGNORE_TOKEN_ID 替换为 tokenizer.unk_token_id
            # rank0_print(conversation)
            # print("#" * 50)
            # rank0_print(tokenizer.decode(z))
...
def preprocess_multi_turn(
    ...
    #print("///////////////////////////////////////*************************************&&&&&&&&&&&&&&&&&&&&&&&222")
    #print(chosen_labels)
    # 将 chosen 变量中的某些词语替换为特殊的掩码标记，以便模型在训练时能够预测这些词语。mask_labels 函数可能会使用对话对象 conv 中的信息来确定哪些词语需要进行掩码处理。
    chosen_labels = mask_labels(chosen, chosen_labels, tokenizer, conv) 
    chosen_labels[:len(prompt_tokens['input_ids'][0])] = IGNORE_TOKEN_ID # 对这部分信息进行忽略
    #print("///////////////////////////////////////*************************************&&&&&&&&&&&&&&&&&&&&&&&333")
    #print(chosen_labels)
    ...
    """print("///////////////////////////////////////*************************************&&&&&&&&&&&&&&&&&&&&&&&333")
    print(rejected_labels.tolist())
    print(rejected_tokens['attention_mask'][0].tolist())
...
def train():
    ...
    #print("!!!!!!!!!!!!!!!!@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@!!!!!!!!!!!!!!!!!!!train_dataset")
    #print(f'1111111chosen_labels: {train_dataset}')
    #print(f'train0: {train_dataset[0]}')
    # dataset:'prompt', 'chosen', 'rejected', 'id', 'chosen_input_ids', 'chosen_attention_mask', 'chosen_labels', 'rejected_input_ids', 'rejected_attention_mask', 'rejected_labels', 'prompt_input_ids', 'prompt_attention_mask'
